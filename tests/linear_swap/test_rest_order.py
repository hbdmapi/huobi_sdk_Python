...
class TestRestOrder(unittest.TestCase):
    ...
    def test_isolated_order(self):
        result = self.api.isolated_order(
            {"contract_code": "eos-usdt", "price": 2.8, "volume": 1, "direction": "buy", "offset": "open", "lever_rate": 10, "order_price_type": "limit"})
        logger.info(result)
        self.assertEqual('ok', result['status'])

    def test_cross_order(self):
        result = self.api.cross_order(
            {"contract_code": "eos-usdt", "price": 2.85, "volume": 1, "direction": "buy", "offset": "open", "lever_rate": 10, "order_price_type": "limit"})
        logger.info(result)
        self.assertEqual('ok', result['status'])

    def test_isolated_batch_order(self):
        result = self.api.isolated_batch_order(
            {"orders_data":[{"contract_code": "eos-usdt", "price": 2.5, "volume": 1, "direction": "buy", "offset": "open", "lever_rate": 10, "order_price_type": "limit"}]})
        logger.info(result)
        self.assertEqual('ok', result['status'])
    ...
    def test_isolated_cancel(self):
        result = self.api.isolated_cancel(
            {"contract_code": "eos-usdt", "order_id": "926438731551268864"})
        logger.info(result)
        self.assertEqual('ok', result['status'])

    def test_cross_cancel(self):
        result = self.api.cross_cancel(
            {"contract_code": "eos-usdt", "order_id": "926438845351124994"})
        logger.info(result)
        self.assertEqual('ok', result['status'])
